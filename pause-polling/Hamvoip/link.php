<?php
	...
	// WD5M - 2022-11-25 - Added optional feature to pause server event polling while browser window is hidden. 
	//                     Add $PAUSE_HIDDEN="YES"; line in global.inc file to enable this pause feature.
	//                     Reformatted indenting and cleaned up some minor errors.
	//                     https://github.com/wd5m/Supermon-mods
	include("session.inc");
	include("global.inc");
	include("common.inc");
	include("header.inc");
...
<?php
	...
	{
?>
		// add event to close/open eventsource when browser window hidden/not-hidden
		if(document.addEventListener) document.addEventListener('visibilitychange', onLoad);
<?php
	}
?>
	...
	function onLoad()
	{
		if(typeof(EventSource)!=="undefined" && !document.hidden)
		{
			//console.log("opened");
			// Start SSE
			source=new EventSource("server.php?nodes=<?php echo $parms; ?>");
							...
							if (nodeNum != 1)
							{
								...
								if (row<ndisp)
								{
									if (sall == "1" || tabledata[localNode].remote_nodes[row].last_keyed != "Never" || total_nodes < 2)
									{
										...
										tablehtml += '<td align="center" id="elap' + row +'">' + tabledata[localNode].remote_nodes[row].elapsed + '</td>';
...
?>
			...
			//console.log("closed");
			if(source.readyState="open") source.close();
<?php
	}
?>
		} else {
...
<?php
			...
			$webport = exec("$CAT /etc/httpd/conf/httpd.conf |$EGREP '^Listen' |$TAIL -1 |$CUT -d' ' -f2 |$SED 's/0.0.0.0://g'");
			if ($myip == $mylanip)
			{
