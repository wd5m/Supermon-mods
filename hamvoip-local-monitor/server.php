...
function parseNode($fp, $rptStatus, $sawStatus) {
    ...
    foreach ($lines as $line) {
        if (preg_match('/Conn: (.*)/', $line, $matches)) {
            ...
            if((is_numeric($arr[0]) && $arr[0] > 3000000) || count($arr) < 6) {	// WD5M 09/2022
                // no ip when echolink
...
// WD5M 9/2022 Added to help set Local Monitor modes on Hamvoip
	if (preg_match('/Var: RPT_ALINKS=(.*)/', $line, $matches)) {
           $arr = preg_split("/,/", trim($matches[1]));
           if(is_numeric($arr[0]) && $arr[0] > 0) {
              ...
              foreach ($arr as $val) {
                 ...
                 $x++;
                 $alinks[substr($val,0,-2)]['mode'] = substr($val,-2);
              }
           }
        }
// END WD5M
    ...
    foreach ($longRangeLinks as $line) {
        $n = substr($line,1);
        if (isset($alinks[$n]['mode'])) {	// WD5M 9/2022
